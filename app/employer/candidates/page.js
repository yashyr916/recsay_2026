          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[['Dashboard', '/employer/dashboard'], ['Post JD', '/employer'], ['Candidates', '/employer/candidates']].map(([l, h]) => (
              <a key={l} href={h} style={{
                fontSize: 13, padding: '6px 14px', borderRadius: 6, textDecoration: 'none', fontWeight: 500,
                background: l === 'Candidates' ? 'rgba(123,47,255,0.15)' : 'transparent',
                color: l === 'Candidates' ? '#C084FC' : 'rgba(255,255,255,0.4)',
                border: l === 'Candidates' ? '1px solid rgba(123,47,255,0.3)' : '1px solid transparent',
              }}>{l}</a>
            ))}
          </div>
        </div>
            ...
            ? (
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
                  {[
                    ['Total',       candidates.length,                                          'white'],
                    ['Shortlisted', candidates.filter(c => c.status === 'shortlisted').length,  '#00D4AA'],
                    ['Hired',       candidates.filter(c => c.status === 'hired').length,        '#FF6B35'],
                    ['Rejected',    candidates.filter(c => c.status === 'rejected').length,     '#FF3C3C'],
                  ].map(([l, n, col]) => (
                ...
                {loading
                  ...
                  : candidates.length === 0
                    ...
                    : candidates.map(c => {
                        ...
                        return (
                          <div key={c.id}
                            style={{ background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20, marginBottom: 12, transition: 'border-color 0.2s' }}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                              ...
                              <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', padding: '4px 10px', borderRadius: 99, background: bg, color: col, border: `1px solid ${border}` }}>
                                {c.status}
                              </span>
                            </div>
                            ...
                            {c.skills?.length > 0 && (
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 10 }}>
                                {c.skills.map(s => (
                                  <span key={s} style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 4, background: 'rgba(123,47,255,0.1)', color: '#C084FC', border: '1px solid rgba(123,47,255,0.2)' }}>{s}</span>
                                ))}
                              </div>
                            )}

                            {c.note && (
                              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '8px 12px', marginBottom: 12, fontStyle: 'italic' }}>
                                "{c.note}"
                              </div>
                            )}
                                ...
                                return (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      background: isActive ? bg2 : 'transparent',
                                      color: isActive ? col2 : 'rgba(255,255,255,0.4)',
