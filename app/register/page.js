  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0A0A0A', display:'flex', fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div className="auth-left" style={{ flex:1, background:'linear-gradient(135deg,#0A1A1A 0%,#0A0A0A 100%)', flexDirection:'column', justifyContent:'center', padding:'60px', position:'relative', overflow:'hidden' }}>
        ...
        <p style={{ fontSize:16, color:'rgba(255,255,255,0.45)', lineHeight:1.7, maxWidth:360, marginBottom:40 }}>
          Whether you&apos;re hiring or recruiting — RecSay&apos;s clustering engine makes you 5× more efficient from day one.
        </p>
      ...
      <div style={{ width:'100%', maxWidth:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'60px 52px', background:'#0D0D0D', borderLeft:'1px solid rgba(255,255,255,0.06)' }}>
        ...
        {error && (
          <div style={{ background:'rgba(255,60,60,0.1)', border:'1px solid rgba(255,60,60,0.3)', borderRadius:8, padding:'12px 16px', marginBottom:20, fontSize:13, color:'#FF6B6B' }}>
            {error}
          </div>
        )}
        ...
        {success ? (
          <div style={{ background:'rgba(0,212,170,0.1)', border:'1px solid rgba(0,212,170,0.3)', borderRadius:12, padding:28, textAlign:'center' }}>
            ...
            <p style={{ color:'rgba(255,255,255,0.5)', fontSize:14 }}>Check your email to confirm, then sign in.</p>
