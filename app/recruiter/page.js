...
export default function RecruiterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: newCandidate, error } = await supabase
      ...
      .select().single();
    if (error) { console.error('Submit error:', error); setLoading(false); return; }
    ...
    try {
      ...
      for (const job of jobs || []) {
        await fetch('/api/send-email', {
          ...
          body: JSON.stringify({
            to: 'yashyr916@gmail.com',
            subject: `New Candidate for ${selectedCluster.title} — RecSay`,
            html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px"><h2 style="color:#7B2FFF">New Candidate Submitted</h2><p>Cluster: <strong>${selectedCluster.title}</strong></p><p><strong>Name:</strong> ${newCandidate.name}</p><p><strong>Email:</strong> ${newCandidate.email}</p><a href="https://recsay.com/employer/candidates" style="background:#7B2FFF;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:16px">Review Candidate →</a></div>`,
          }),
        });
      }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0A0A0A', fontFamily:"'DM Sans',sans-serif", color:'white' }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'48px 40px' }}>
        <div style={{ marginBottom:32 }}>
          ...
          <p style={{ fontSize:14, color:'rgba(255,255,255,0.4)' }}>
            {filtered.length} cluster{filtered.length!==1?'s':''} found{search && ` for "${search}"`} — click a card to view full JD
          </p>
        </div>
        ...
        {paginated.length === 0
          ? <div style={{ background:'#111', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:60, textAlign:'center' }}>
              <p style={{ color:'rgba(255,255,255,0.35)', fontSize:15 }}>No clusters found.</p>
                ...
                {paginated.map(cluster => (
                  <div key={cluster.id}
                      ...
                      <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
                        {(cluster.keywords||[]).slice(0,7).map(k => (
                          <span key={k} style={{ fontSize:11, fontWeight:600, padding:'3px 8px', borderRadius:4,
                            background:'rgba(123,47,255,0.15)', color:'#C084FC', border:'1px solid rgba(123,47,255,0.25)' }}>{k}</span>
                        ))}
                    ...
                    <div style={{ padding:'14px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); setSelectedCluster(cluster); setShowForm(true); setSuccess(false); }}
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i+1).map(n => (
                    <button key={n} onClick={() => setPage(n)}
                      style={{ width:36, height:36, borderRadius:8, border:'none', cursor:'pointer', fontSize:13, fontWeight:600, fontFamily:"'DM Sans',sans-serif", transition:'all 0.2s',
                        ...
                        color: page===n ? 'white' : 'rgba(255,255,255,0.4)' }}>{n}</button>
                  ))}
      ...
      {detailPanel && (
        <>
          <div onClick={() => setDetailPanel(null)}
          ...
          <div style={{ position:'fixed', top:0, right:0, bottom:0, width:'min(520px,95vw)',
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24 }}>
              <div>
                <div style={{ fontSize:10, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:'#7B2FFF', marginBottom:6 }}>Cluster Detail</div>
                <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:24, fontWeight:800, color:'white', marginBottom:4 }}>{detailPanel.title}</h2>
                <p style={{ fontSize:13, color:'rgba(255,255,255,0.35)' }}>
                  {detailPanel.job_ids?.length || 0} employer{(detailPanel.job_ids?.length||0)!==1?'s':''} hiring for this role
                </p>
              </div>
            ...
            <div style={{ marginBottom:24 }}>
              <div style={{ fontSize:11, fontWeight:600, color:'rgba(255,255,255,0.35)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:10 }}>Required Skills</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                {(detailPanel.keywords||[]).map(k => (
                  <span key={k} style={{ fontSize:12, fontWeight:600, padding:'4px 10px', borderRadius:4, background:'rgba(123,47,255,0.15)', color:'#C084FC', border:'1px solid rgba(123,47,255,0.25)' }}>{k}</span>
                ))}
              </div>
            </div>
            <div style={{ marginBottom:28 }}>
              <div style={{ fontSize:11, fontWeight:600, color:'rgba(255,255,255,0.35)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:12 }}>
                Job Descriptions ({panelJobs.length})
              </div>
              {panelLoading
                ? <div style={{ color:'rgba(255,255,255,0.3)', fontSize:13 }}>Loading...</div>
                : panelJobs.map((job, i) => (
                  <div key={job.id} style={{ background:'#111', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:18, marginBottom:12 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                      <div style={{ fontSize:14, fontWeight:700, color:'white' }}>Role #{i+1} — {job.title}</div>
                      <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:4, background:'rgba(123,47,255,0.15)', color:'#C084FC', textTransform:'uppercase' }}>{job.type}</span>
                    </div>
                    <p style={{ fontSize:13, color:'rgba(255,255,255,0.5)', lineHeight:1.65, whiteSpace:'pre-wrap' }}>{job.description}</p>
                  </div>
                ))
              }
            </div>
            <button onClick={() => { setSelectedCluster(detailPanel); setDetailPanel(null); setShowForm(true); setSuccess(false); }}
              style={{ width:'100%', padding:'14px', background:'linear-gradient(135deg,#7B2FFF,#9B5FFF)', color:'white', border:'none', borderRadius:10, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
              Submit Candidate to {detailPanel.job_ids?.length||0} Employer{(detailPanel.job_ids?.length||0)!==1?'s':''} →
            </button>
          </div>
        </>
      )}
